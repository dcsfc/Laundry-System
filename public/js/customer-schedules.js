...
function submitScheduleForm() {
    ...
    const requiredFields = [
        ...
        { field: 'dropoff_time', message: 'Please select a drop-off time' },
        { field: 'pickup_date', message: 'Please select a pickup date' },
        { field: 'pickup_time', message: 'Please select a pickup time' }
    ];
...
function submitEditForm() {
    ...
    const dropoffTime = formData.get('dropoff_time');
    const pickupTime = formData.get('pickup_time');
    
    if (!dropoffTime || dropoffTime === '') {
        showToast('Please select a valid drop-off time', 'error');
        return;
    }
    
    if (!pickupTime || pickupTime === '') {
        showToast('Please select a valid pickup time', 'error');
        return;
    }
    
    // If same day, ensure pickup time is after dropoff time
    if (dropoffDate.getTime() === pickupDate.getTime()) {
        const dropoffTimeMinutes = parseInt(dropoffTime.split(':')[0]) * 60 + parseInt(dropoffTime.split(':')[1]);
        const pickupTimeMinutes = parseInt(pickupTime.split(':')[0]) * 60 + parseInt(pickupTime.split(':')[1]);
        
        if (pickupTimeMinutes <= dropoffTimeMinutes) {
            showToast('Pickup time must be after drop-off time when scheduled on the same day', 'error');
            return;
        }
    }
    
    // Submit form
    const updateRoute = window.scheduleUpdateRoute.replace(':id', scheduleId);
    fetch(updateRoute, {
        method: 'POST',
        body: formData,
        headers: {
...
function submitDeleteForm() {
    ...
    // Submit form
    const deleteRoute = window.scheduleDeleteRoute.replace(':id', scheduleId);
    fetch(deleteRoute, {
        method: 'POST',
        body: formData,
        headers: {
...
function initializeFormValidation() {
    ...
    // Add validation for time fields (select elements)
            ...
            if (form) {
                ...
                if (dateInput && dateInput.value === now.toISOString().split('T')[0]) {
                    if (this.value < currentTime) {
                        showToast('Time cannot be in the past', 'error');
                        this.value = '';
                    }
                }
            }
