...
@if(config('app.debug'))
<script>
    console.log('Pending schedules data:', @json($pendingSchedules));
    console.log('Approved schedules data:', @json($approvedSchedules));
</script>
@endif
...
<div class="space-y-8">
        ...
        <div class="bg-gradient-to-br from-amber-500/20 to-orange-500/20 border border-amber-500/30 rounded-xl p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-white">{{ $pendingSchedules->count() }}</p>
                </div>
    ...
    <div class="table-container" 
         x-data="pendingSchedulesTable(@js($pendingSchedules), 10)"
         x-init="init()">
        ...
        <!-- Search and Filters -->
        @include('components.tables.search-filters', ['showRoleFilter' => false])
            ...
            <table class="data-table">
                <thead class="table-head">
                    <tr>
                        ...
                        @foreach($pendingColumns as $column)
                ...
                <tbody>
                    ...
                    <template x-if="isLoading">
                        <tr>
                            <td :colspan="{{ count($pendingColumns) + 1 }}" class="text-center py-8">
                    ...
                    <template x-for="(row, index) in paginatedData" :key="row.id || index">
                        @include('components.tables.schedule-row', ['columns' => $pendingColumns])
                    </template>
                </tbody>
                ...
                <tfoot x-show="paginatedData.length === 0 && !isLoading">
                    <tr>
                        <td :colspan="{{ count($pendingColumns) + 1 }}" class="empty-content">
        ...
        <!-- Pagination -->
        @include('components.tables.pagination', ['itemName' => 'schedules'])
    </div>
