/**
 * Action Menu Manager - Professional SaaS Data Table Actions
 * 
 * Handles the creation, positioning, and management of action menus
 * for data table rows. Provides a portal-based system for optimal
...
class ActionMenuManager {
    ...
    /**
     * Setup the portal system for rendering action menus
     * Creates a fixed positioned container for optimal z-index management
     */
    setupPortalSystem() {
    ...
    /**
     * Open an action menu for a specific row
     ...
     * @param {Function} onAction - Callback function when an action is clicked
     */
    ...
    /**
     * Render the action menu in the portal
     * @param {Object} rowData - Data object for the row
     * @param {Array} actions - Array of action objects
     * @param {Function} onAction - Callback function when an action is clicked
     */
    renderPortalMenu(rowData, actions, onAction) {
        ...
        // Position menu to the right of trigger, or left if not enough space
        ...
        // Position menu below trigger, or above if not enough space
    ...
    /**
     * Close the currently open action menu
     */
    closeActionMenu() {
        const portal = document.getElementById(this.portalId);
        if (portal) {
    ...
    /**
     * Get SVG icon path for action type
     * @param {string} icon - Icon type identifier
    ...
    getActionIcon(icon) {
        const icons = {
            ...
            archive: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8l6 6 6-6M5 8h14M5 8l6-6 6 6"></path>',
            restore: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"></path>'
        };
    ...
    /**
     * Check if a menu is currently open for a specific row
     * @param {string} rowId - Row identifier
     * @returns {boolean} True if menu is open for this row
     */
    isMenuOpen(rowId) {
    ...
    /**
     * Get the currently active menu row ID
     * @returns {string|null} Active row ID or null
     */
    getActiveMenuRow() {
    ...
    /**
     * Destroy the action menu manager and clean up
     */
    destroy() {
        this.closeActionMenu();
        const portal = document.getElementById(this.portalId);
